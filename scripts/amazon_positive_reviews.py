...
see_all_reviews = driver.find_element_by_xpath("//div//a[@data-hook = 'see-all-reviews-link-foot']")
...
recent_reviews= driver.find_element_by_xpath("/html/body/div[3]/div/div/ul/li[8]/a")
# clicking positive reviews
...
my_positive_dict ={"date":[] , "star": [], "heading" : [] , "review" :[]}

# function to get data from pages 
def scrap_pages_pos(page):
    global my_positive_dict
    ...
    for parts in blocks:
        ...
        my_positive_dict["date"].append(date)
        my_positive_dict["star"].append(rating)
        my_positive_dict["heading"].append(head)
        my_positive_dict["review"].append(review)
...
scrap_pages_pos(first_page_selector)
while(x< 10 ):
    ...
    scrap_pages_pos(second_page_selector)
    x = x+1



final_data= pd.DataFrame.from_dict(my_positive_dict)
final_data.to_csv("positive_amazon.csv")
